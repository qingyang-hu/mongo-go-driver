...
type MongoCryptOptions struct {
	...
	AWSCredentialProvider      credentials.Provider
}
...
// SetAWSCredentialProvider sets the custom AWS credential provider.
func (mo *MongoCryptOptions) SetAWSCredentialProvider(provider credentials.Provider) *MongoCryptOptions {
	mo.AWSCredentialProvider = provider
	return mo
}
